"""Schema/quality gate for `agents/evals/f2/golden.jsonl` (phase-03 §8: "25
golden turns"). Per the revised testing policy and docs/decisions/0015's
deferral precedent (reapplied here for F2), this is schema-only:
`iam_sentinel_agents.evals.runner` (phase-12) doesn't exist yet and no
deployed Prime/F2 agent exists to actually run a turn against.
"""
...
GOLDEN_PATH = Path(__file__).resolve().parents[2] / "evals" / "f2" / "golden.jsonl"
...
_VALID_SEVERITIES = {None, "INFO", "LOW", "MEDIUM", "HIGH", "CRITICAL"}
